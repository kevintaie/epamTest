...
{
    private readonly BudgetWingsProvider _provider =
        new(NullLogger<BudgetWingsProvider>.Instance);
    ...
    [Fact]
    public async Task Results_differ_from_GlobalAir_for_same_route()
    {
        var globalAir = new GlobalAirProvider(NullLogger<GlobalAirProvider>.Instance);

        var budget = (await _provider.SearchAsync("EZE", "MIA", FutureDate, 1, CabinClass.Economy))
            .Select(f => f.FlightNumber).ToList();
        var global = (await globalAir.SearchAsync("EZE", "MIA", FutureDate, 1, CabinClass.Economy))
            .Select(f => f.FlightNumber).ToList();

        budget.Should().NotBeEquivalentTo(global,
            because: "each provider uses a different seed");
    }

    [Fact]
    public async Task All_prices_are_at_least_minimum_floor()
    {
        const decimal minimumPrice = 29.99m;
    ...
    public async Task Domestic_flights_have_short_duration()
    {
        ...
        flights.Should().AllSatisfy(f =>
            f.Duration.TotalHours.Should().BeLessThan(4,
    ...
    [Fact]
    public async Task Provider_name_is_BudgetWings()
    {
        _provider.ProviderName.Should().Be("BudgetWings");
    }
    ...
    public async Task First_class_costs_more_than_business()
    {
        ...
        firstClass.Should().BeGreaterThan(business,
            because: "FirstClass multiplier (4.2x) is higher than Business (2.4x)");
    }
